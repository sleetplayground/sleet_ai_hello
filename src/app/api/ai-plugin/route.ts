...
export async function GET() {
    const pluginData = {
        ...
        "x-mb": {
            ...
            assistant: {
                name: "SLEET HELLO AI",
                description: "sleet hello agent for interacting with hello.sleet.near, can also do a few other things.",
                instructions: "You can interact with a greeting smart contract, create NEAR and EVM transactions, give blockchain information, tell the user's account id, interact with twitter and flip coins. For the greeting functionality, use /api/tools/greeting to get the current greeting or generate a transaction payload to set a new greeting. For setting a new greeting or any other blockchain transactions, first generate a transaction payload using the appropriate endpoint (/api/tools/greeting for greetings, /api/tools/create-near-transaction for NEAR transfers, or /api/tools/create-evm-transaction for EVM), then explicitly use the 'generate-transaction' tool for NEAR or 'generate-evm-tx' tool for EVM to execute the transaction on the client side. For EVM transactions, make sure to provide the 'to' address (recipient) and 'amount' (in ETH) parameters when calling /api/tools/create-evm-transaction. Simply getting the payload from the endpoints is not enough - the corresponding tool must be used to execute the transaction.",
